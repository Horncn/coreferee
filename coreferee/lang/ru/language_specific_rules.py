...
class LanguageSpecificRulesAnalyzer(RulesAnalyzer):
    ...
    def get_dependent_siblings(self, token: Token) -> list:
        ...
        def add_siblings_recursively(recursed_token: Token, visited_set: set) -> None:
            ...
            if token != recursed_token and token.pos_ in ('VERB', 'AUX') and \
                    recursed_token.pos_ in ('VERB', 'AUX') and \
                    self.is_potential_anaphor(recursed_token) and \
                    self.is_potential_anaphor(token):
            ...
            for child in (child for child in recursed_token.children if child not in visited_set and
                    (child.dep_ in self.dependent_sibling_deps or
                     child.dep_ in self.conjunction_deps)):
            ...
            for child in (child for child in recursed_token.children if recursed_token.pos_ in
                            ...
                            self.has_morph(child, 'Case', 'Ins') and
    ...
    def is_potential_anaphoric_pair(self, referred: Mention, referring: Token, directly: bool) \
            -> int:
        ...
        comitative_siblings = [c for c in referring._.coref_chains.temp_dependent_siblings if
                               referring.pos_ in ('VERB', 'AUX') and
                               self.has_morph(referring, 'Number', 'Plur') and
                               not (self.has_morph(referring, 'Person', 'First') or
        ...
        if len(all_involved_referreds) > 1:
            ...
            referred_comitative_siblings = [child for child in
        ...
        for working_token in (doc[index] for index in referred.token_indexes):
            working_masc, working_fem, working_neut = get_gender_number_info(working_token)
            ...
            if referred_root.dep_ not in self.dependent_sibling_deps:
                if self.has_morph(referring, 'Number', 'Plur') or \
                    ...
                    if not self.has_morph(referred_root, 'Case', 'Ins') and \
                            ...
                            self.has_morph(referred_root.head, 'Animacy', 'Anim') and \
                            not [child for child in referred_root.children if \
                                 child.dep_ == 'case' and child.lemma_ == 'c']:
                        if not self.is_reflexive_possessive_pronoun(referring):
                            return 0

                if not [child for child in referred_root.children if
                        child.dep_ in self.dependent_sibling_deps] and \
                        not self.is_reflexive_possessive_pronoun(referring):
                    if referred_root.pos_ == 'PRON':
                        if not [child for child in referred_root.head.children if
                                child.dep_ == 'obj'] or \
                                not [child for child in referred_root.head.children if
                                     child.dep_ == 'obl']:
                            return 0
                    else:
                        return 0
        ...
        if referred_root.dep_ == 'obj':
            if [child for child in referred_root.children if
                child.dep_ in self.dependent_sibling_deps and \
                get_gender_number_info(child) == get_gender_number_info(referred_root)]:
        ...
        if referred_root.dep_ == 'nsubj' and referred_root.head.dep_ == self.root_dep:
            if referring.lemma_.startswith('котор') and \
                    not self.has_morph(referring, 'Case', 'Gen'):
                if [child for child in referred_root.head.children if
                    child.dep_ in ('obj', 'obl') and
                    get_gender_number_info(child) == get_gender_number_info(referred_root) and
                    child.i < referring.i]:
                    return 0

            if [child for child in referred_root.children if child.dep_ == 'nmod' and
                                                             child.i > referred_root.i]:
                if not self.is_reflexive_possessive_pronoun(referring):
                    return 0
        ...
        if referred_root.dep_ == self.root_dep:
            ...
            if len([child for child in referred_root.children]) == \
                    len([child for child in referred_root.children if child.dep_ == 'punct']):
                return 0

            if [child for child in referred_root.children if child.dep_ == 'case'] and \
                    [child for child in referred_root.children if
                     child.dep_ == 'nmod' and child.i > referred_root.i]:
                return 0

        if self.has_morph(referred_root, 'Animacy', 'Inan') and \
                self.has_morph(referring, 'Person', 'Third'):
            if [child for child in referred_root.head.children if
                    self.has_morph(child, 'Animacy', 'Anim') and child != referring]:
        ...
        if referred_root.head.pos_ == 'NOUN' and referred_root.dep_ == 'nmod':
            if not (self.has_morph(referred_root, 'Animacy', 'Anim') and
                    self.has_morph(referred_root.head, 'Animacy', 'Inan')):
                if not [child for child in referred_root.children if child.pos_ == 'ADP']:
                    return 0

        if referred_root.pos_ == 'NOUN' and referring.dep_ == 'obj':
            if not self.has_morph(referred_root, 'Animacy', 'Anim'):
                if [child for child in referred_root.children if child.dep_ == 'nmod' and
                                                        self.has_morph(child, 'Animacy', 'Anim')]:
                    return 0
            else:
                if [child for child in referred_root.children if child.dep_ == 'nmod']:
                    if not self.is_reflexive_possessive_pronoun(referring):
                        return 0
        ...
        if referred_root.lemma_.endswith('.') and referred_root.pos_ in self.propn_pos and \
                self.has_morph(referred_root, 'Animacy', 'Anim'):
            return 0
        ...
        if referring.pos_ == 'DET':
            if referring.lemma_.startswith('наш'):
                if not (referred_root.lemma_.startswith('наш') or
                        referred_root.lemma_.startswith('мы')):
                    return 0

            if referring.lemma_.startswith('ваш'):
                if not (referred_root.lemma_.startswith('ваш') or
                        referred_root.lemma_.startswith('вы')):
                    return 0

            if referring.head.pos_ == 'NOUN':
                if not self.is_reflexive_possessive_pronoun(referring) and \
                        not self.has_morph(referring, 'Case', 'Nom'):
                    return 0
        ...
        if not self.is_reflexive_possessive_pronoun(referring):
            ...
            if (self.has_morph(referring, 'Person', 'First') and
                not self.has_morph(referred_root, 'Person', 'First')) or \
                    (self.has_morph(referred_root, 'Person', 'First') and
                     not self.has_morph(referring, 'Person', 'First')):
                return 0

            if (self.has_morph(referring, 'Person', 'Second') and
                not self.has_morph(referred_root, 'Person', 'Second')) or \
                    (self.has_morph(referred_root, 'Person', 'Second') and
                     not self.has_morph(referring, 'Person', 'Second')):
                return 0

        if self.is_reflexive_possessive_pronoun(referred_root):
            if referred_root.head.pos_ == 'NOUN' and referred_root.head.dep_ == 'obj' and \
                    self.has_morph(referred_root.head, 'Case', 'Acc'):
                return 0

            if self.has_morph(referred_root, 'Case', 'Loc') and \
                    get_gender_number_info(referred_root.head) == get_gender_number_info(referring):
                return 0

        if self.has_morph(referred_root, 'Case', 'Nom'):
            if referred_root.head.lemma_ in self.verbs_with_personal_subject:
                if [child for child in referred_root.head.children if child.tag_ in self.noun_pos]:
                    return 0

        if self.has_morph(referred_root, 'Animacy', 'Anim'):
            if [child for child in referred_root.children if child.dep_ == 'amod' and \
                                                             self.has_morph(child, 'Animacy', 'Anim')]:
                return 0

        if [child for child in referred_root.children if child.dep_ in self.dependent_sibling_deps]:
            if self.has_morph(referring, 'Number', 'Sing') and \
                    self.has_morph(referring.head, 'Number', 'Plur'):
                return 0
        ...
        if referred_root.sent.text != referring.sent.text:
            ...
            if ' '.join([referred_root.sent.text, referring.sent.text]) not in \
                    doc.text and referred_root.i < referring.i:
                ...
                while True:
                    ...
                    if self.is_independent_noun(temp_token):
                        if self.is_potential_anaphoric_pair(Mention(doc[temp_token.i], False),
                                                            referring, directly):
                            return 0
    ...
    def is_potentially_indefinite(self, token: Token) -> bool:
        ...
        for child in (child for child in token.children if child.pos_ in self.term_operator_pos):
            ...
            if child.pos_ == 'DET' and child.tag_ == 'ADJ' and child.dep_.startswith('det') and \
                ...
                return False
            if child.pos_ == 'DET' and child.tag_ == 'DET':
                return False
        return True
    ...
    def is_reflexive_anaphor(self, token: Token) -> int:
        ...
        if token.tag_ == 'PRON':
            if token.dep_ == 'nmod' and self.has_morph(token, 'Case', 'Acc'):
                return 1
            if token.dep_ == 'obl' and self.has_morph(token, 'Case', 'Ins') and \
                    not self.has_morph(token, 'Person', 'Third'):
    ...
    def is_potential_reflexive_pair(self, referred: Mention, referring: Token) -> bool:
        ...
        if referred_root.dep_.startswith('nsubj') or (referred_root.pos_ in ('VERB', 'AUX') and
            ...
            for referring_or_ancestor in referring_and_ancestors:
                ...
                if referring_or_ancestor.pos_ in ('VERB', 'AUX') and \
                        ...
                        and not referring.lemma_.startswith('котор'):
                    return True

                # The ancestor has its own subject, so stop here
                if [t for t in referring_or_ancestor.children if t.dep_.startswith('nsubj') and
                                                                      t != referred_root]:
                    return False
        ...
        return referring.dep_ != self.root_dep and referred_root.dep_ != self.root_dep and \
               (referring.head == referred_root.head or
                referring.head.i in referred.token_indexes) and \
                referring.i > referred_root.i
