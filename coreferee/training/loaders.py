...
class RuCorefLoader(GenericLoader):    
    ...
    def load(self, directory_name:str, nlp:Language, rules_analyzer:RulesAnalyzer) -> list:
        ...
        for folder in os.listdir(directory_name):
            folder = os.sep.join([directory_name, folder])
            if os.path.isdir(folder):
                for index, txt_filename in enumerate(t for t in os.listdir(folder) if
            t.endswith('.txt')):   
                    full_text_filename = os.sep.join([folder, txt_filename])
